...
const AdminGeneral = () => {
    ...
    const handleApproveReset = async () => {
        ...
        try {
            ...
            toast.success(
                isAr
                    ? 'تمت الموافقة! سيظهر الباركود الجديد عند دخول المشرف في المرة القادمة.'
    ...
    if (loading) {
        return (
            <div className="admin-loading-container">
                ...
                <p>{isAr ? 'جاري تحميل الإعدادات العامة...' : 'Loading general settings...'}</p>
            </div>
        );
    }
            ...
            {!resetLoading && resetRequest && resetRequest.status === 'pending' && (
                <div style={{
                    ...
                    <div style={{ display: 'flex', gap: '0.6rem' }}>
                        ...
                        >
                            {isAr ? 'موافقة وتوليد باركود جديد' : 'Approve & Generate New QR'}
                        </button>
                        <button
                ...
                <div className="admin-glass-card">
                    ...
                    <div className="admin-setting-item">
                        <div className="setting-details">
                            ...
                            <p>{isAr ? 'تفعيل أو إيقاف نافذة التقييم التلقائية للزوار الجدد.' : 'Enable/disable rating popup for new visitors.'}</p>
                        </div>
                        <button 
                            ...
                            disabled={saving}
                        >
                            {settings.feedbackPopupEnabled ? (isAr ? 'مفعّلة' : 'Enabled') : (isAr ? 'معطّلة' : 'Disabled')}
                        </button>
                    </div>
                ...
                <div className="admin-glass-card" style={{ marginTop: '1.5rem' }}>
                    ...
                    <div className="admin-setting-item">
                        <div className="setting-details">
                            ...
                            <p>{isAr ? 'تحديد المرحلة النشطة لحملة تبادل الكتب.' : 'Set current campaign phase.'}</p>
                        </div>
                        <select 
                    ...
                    <div className="admin-setting-item-block">
                        <h5>{isAr ? 'رسالة توقف الحملة (بالعربية)' : 'Suspended Message (Arabic)'}</h5>
                        <textarea
                        ...
                        <button 
                            ...
                            disabled={saving}
                        >
                            {isAr ? 'حفظ الرسالة العربية' : 'Save Message'}
                        </button>
                    </div>
                    ...
                    <div className="admin-setting-item-block">
                        <h5>{isAr ? 'رسالة توقف الحملة (بالإنجليزية)' : 'Suspended Message (English)'}</h5>
                        <textarea
                            className="admin-textarea-field"
                        ...
                        <button 
                            ...
                            disabled={saving}
                        >
                            {isAr ? 'حفظ الرسالة الإنجليزية' : 'Save Message'}
                        </button>
                    </div>
                </div>
            </div>
